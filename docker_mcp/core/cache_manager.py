...
class ContainerCache:
    ...
    compose_working_dir: Optional[str]   # Working directory from labels
...
class DockerCacheManager:
    ...
    def __init__(self, config: DockerMCPConfig, context_manager: DockerContextManager, cache_dir: Optional[Path] = None):
        self.config = config
        ...
        # Cache storage
        self.containers_cache: Dict[str, Dict[str, ContainerCache]] = {}  # host_id -> container_id -> cache
    ...
    async def _initial_cache_warmup(self):
        ...
        try:
            ...
            self.logger.info(f"Cache warmup completed: {total_containers} containers across {len(hosts)} hosts")
    ...
    async def _update_host_cache(self, host_id: str, host_config: Dict):
        ...
        try:
            ...
            if not client:
                self.logger.warning(f"Could not connect to host {host_id} - no Docker client available")
                return
    ...
    async def _build_container_cache(self, host_id: str, container, stats: Dict) -> ContainerCache:
        ...
        stack_containers = []
        if compose_project:
            stack_containers = await self._get_compose_stack_containers(
    ...
    async def get_containers(self, host_id: str, force_refresh: bool = False) -> List[ContainerCache]:
    ...
    async def get_container(self, host_id: str, container_id: str, force_refresh: bool = False) -> Optional[ContainerCache]:
    ...
    async def find_by_label(self, label_key: str, label_value: Optional[str] = None) -> List[ContainerCache]:
    ...
    async def cross_host_search(self, query: str) -> List[ContainerCache]:
        ...
        for host_id, containers in self.containers_cache.items():
            for container in containers.values():
                if (query_lower in container.name.lower() or
                    query_lower in container.image.lower() or
                    container.container_id.startswith(query)):
                    results.append(container)
            ...
            try:
                ...
                for host_id in list(self.cache_timestamps.keys()):
                    ...
                    if age > self.cache_config.CONFIG_RETENTION:
                        self.logger.info(f"Cleaning up stale cache for host {host_id} (age: {age:.0f}s)")
            ...
            for cache_file in self.cache_dir.glob("*_cache.pkl.gz"):
                try:
                    ...
                    if cache_age < 3600:  # 1 hour
                        ...
                        self.logger.debug(f"Loaded cache for {host_id} with {len(cache_bundle['containers'])} containers")
                    else:
                        self.logger.debug(f"Cache for {cache_file.stem} too old ({cache_age:.0f}s), skipping")
                        
                except Exception as e:
                    self.logger.warning(f"Failed to load cache from {cache_file}: {e}")
            
            if loaded_hosts > 0:
                self.logger.info(f"Cache warmup completed: loaded {total_containers} containers from {loaded_hosts} hosts")
            else:
...
_cache_manager: Optional[DockerCacheManager] = None

async def get_cache_manager(config: DockerMCPConfig, context_manager: DockerContextManager) -> DockerCacheManager:
