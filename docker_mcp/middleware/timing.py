...
class TimingMiddleware(Middleware):
    ...
    async def _update_statistics(self, method: str, duration_ms: float, success: bool) -> None:
        ...
        self.request_times[method].append(
            {"duration_ms": duration_ms, "success": success, "timestamp": time.time()}
        )
        ...
        if method_times:
            self.method_stats[method] = {
                ...
                "success_rate": sum(1 for req in self.request_times[method] if req["success"])
                / len(method_times),
                "slow_count": sum(1 for t in method_times if t > self.slow_threshold_ms),
            }

    async def _log_timing(
        self, method: str, duration_ms: float, success: bool, context: MiddlewareContext
    ) -> None:
    ...
    def get_performance_statistics(self) -> dict[str, Any]:
        ...
        if all_durations:
            overall_stats = {
                ...
                "median_duration_ms": sorted(all_durations)[len(all_durations) // 2]
                if all_durations
                else 0,
            }
