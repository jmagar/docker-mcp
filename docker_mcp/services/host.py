...
class HostService:
    ...
    async def edit_docker_host(
        ...
        try:
            ...
            updated_config = {
                ...
                "identity_file": ssh_key_path if ssh_key_path is not None else current_host.identity_file,
                ...
                "compose_path": compose_path if compose_path is not None else current_host.compose_path,
                "appdata_path": appdata_path if appdata_path is not None else current_host.appdata_path,
            ...
            return {
                ...
                "host_id": host_id,
                "updated_fields": [k for k, v in locals().items() 
                                 if k.startswith(('ssh_', 'description', 'tags', 'compose_', 'appdata_', 'enabled')) 
                                 and v is not None],
            }
    ...
    async def test_connection(self, host_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            ssh_cmd.append("echo 'connection_test_ok' && docker version --format '{{.Server.Version}}' 2>/dev/null || echo 'docker_not_available'")
    ...
    async def discover_host_capabilities(self, host_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            compose_result = results[0] if not isinstance(results[0], Exception) else {"paths": [], "recommended": None}
            appdata_result = results[1] if not isinstance(results[1], Exception) else {"paths": [], "recommended": None}
            ...
            if zfs_result["capable"]:
                ...
                if "zfs" not in host.tags:
                    ...
                    try:
                        ...
                        self.logger.error("Failed to save config after adding zfs tag", host_id=host_id, error=str(e))
                        tag_added = False
                else:
    ...
    async def discover_all_hosts(self) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info("Starting discovery for all hosts", total_hosts=len(host_ids), host_ids=host_ids)
    ...
    async def _discover_compose_paths_ssh(self, host: DockerHost) -> dict[str, Any]:
        ...
        try:
            ...
            inspect_cmd = ssh_cmd + [
                "docker ps -aq --no-trunc | xargs -r docker inspect --format '{{index .Config.Labels \"com.docker.compose.project.working_dir\"}}' 2>/dev/null | grep -v '^$' | sort | uniq"
            ]
            ...
            if process.returncode == 0 and stdout:
                ...
                if compose_dirs:
                    ...
                    recommended = max(path_counts.items(), key=lambda x: x[1])[0] if path_counts else None
            ...
            self.logger.warning("Failed to discover compose paths", hostname=host.hostname, error=str(e))
            return {"paths": [], "recommended": None}
        ...
        try:
            ...
            if all_bind_mounts:
                ...
                if base_path_counts:
                    ...
                    recommended = base_path_counts.most_common(1)[0][0] if base_path_counts else None
    ...
    async def _discover_appdata_paths_ssh(self, host: DockerHost) -> dict[str, Any]:
        ...
        try:
            ...
            search_paths = ["/opt/appdata", "/srv/docker", "/data", "/mnt/appdata", "/mnt/docker", "/opt/docker-data"]
            ...
            self.logger.warning("Failed to discover appdata paths", hostname=host.hostname, error=str(e))
            return {"paths": [], "recommended": None}
    ...
    def _build_ssh_cmd(self, host: DockerHost) -> list[str]:
        """Build SSH command for host connection."""
    ...
    async def _test_ssh_connection(self, hostname: str, user: str, port: int = 22, identity_file: str | None = None) -> bool:
        ...
        try:
            ...
            self.logger.error(
                ...
                hostname=hostname, user=user, port=port, error=str(e)
            )
            return False
