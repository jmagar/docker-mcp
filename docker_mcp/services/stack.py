...
class StackService:
        ...
        try:
            ...
            if result.returncode != 0:
                return ToolResult(
                    content=[
                        TextContent(
                            type="text", text=f"Error: Failed to read compose file: {result.stderr}"
                        )
                    ],
                    structured_content={
                        "success": False,
                        "error": f"Compose file not found: {result.stderr}",
                    },
                )
            ...
            is_valid, validation_issues, validation_details = self._validate_compose_syntax(
            ...
            if is_valid:
                ...
                migration_steps.append(
                    f"   ✅ Compose file is valid ({services_count} services found)"
                )
                ...
                migration_steps.append(
                    f"   ❌ Compose file validation failed ({len(validation_issues)} issues)"
                )
                ...
                if len(validation_issues) > 3:
                    migration_steps.append(
                        f"     • ... and {len(validation_issues) - 3} more issues"
                    )
                ...
                if not dry_run:
                    ...
                    return ToolResult(
                        content=[
                            TextContent(
                                type="text",
                                text=f"❌ Migration blocked: Compose file validation failed with {len(validation_issues)} issues",
                            )
                        ],
                        structured_content={
                            "success": False,
                    ...
                    migration_steps.append(
                        "⚠️  (DRY RUN) Migration would fail due to compose validation errors"
                    )

            # Step 2: Parse volumes from compose
            migration_steps.append("🔍 Analyzing volume configuration...")
            volumes_info = await self.migration_manager.parse_compose_volumes(
                compose_content, source_appdata
            )
            ...
            if all_paths:
                ...
                size_cmd = ssh_cmd_source + [
                    f"du -sb {' '.join(shlex.quote(p) for p in all_paths)} 2>/dev/null | awk '{{sum+=$1}} END {{print sum}}'"
                ]
                ...
                has_space, space_message, space_details = await self._check_disk_space(
                    target_host, estimated_archive_size
                )
                migration_steps.append(f"   {space_message}")
                migration_steps.append(
                    f"   Raw data: {format_size(estimated_size)} → Archive: ~{format_size(estimated_archive_size)}"
                )
                ...
                if not has_space and not dry_run:
                    ...
                    return ToolResult(
                        content=[
                            TextContent(type="text", text=f"❌ Migration blocked: {space_message}")
                        ],
                        structured_content={
                            "success": False,
                    ...
                    # For dry runs, show as warning
                    migration_steps.append(
                        "⚠️  (DRY RUN) Migration would fail due to insufficient disk space"
                    )
            else:
            ...
            source_available, source_missing, source_details = await self._check_tool_availability(
                source_host, required_tools
            )
            if source_available:
                migration_steps.append(
                    f"   ✅ Source ({source_host.hostname}): All tools available"
                )
            else:
                migration_steps.append(
                    f"   ❌ Source ({source_host.hostname}): Missing {', '.join(source_missing)}"
                )

            # Check target host tools
            target_available, target_missing, target_details = await self._check_tool_availability(
                target_host, required_tools
            )
            if target_available:
                migration_steps.append(
                    f"   ✅ Target ({target_host.hostname}): All tools available"
                )
            else:
                migration_steps.append(
                    f"   ❌ Target ({target_host.hostname}): Missing {', '.join(target_missing)}"
                )
            ...
            if not tools_check_passed and not dry_run:
                ...
                return ToolResult(
                    content=[
                        TextContent(
                            type="text",
                            text=f"❌ Migration blocked: Missing required tools: {', '.join(all_missing)}",
                        )
                    ],
                    structured_content={
                        "success": False,
                ...
                all_missing = list(set(source_missing + target_missing))
                migration_steps.append(
                    f"⚠️  (DRY RUN) Migration would fail due to missing tools: {', '.join(all_missing)}"
                )
            ...
            if exposed_ports:
                migration_steps.append(
                    f"   Found {len(exposed_ports)} exposed ports: {', '.join(map(str, exposed_ports))}"
                )
                no_conflicts, conflicted_ports, conflict_details = await self._check_port_conflicts(
                    target_host, exposed_ports
                )
                ...
                if no_conflicts:
                    ...
                    migration_steps.append(
                        f"   ❌ Port conflicts on {target_host.hostname}: {', '.join(map(str, conflicted_ports))}"
                    )
                    ...
                    if not dry_run:
                        ...
                        return ToolResult(
                            content=[
                                TextContent(
                                    type="text",
                                    text=f"❌ Migration blocked: Port conflicts on target host: {', '.join(map(str, conflicted_ports))}",
                                )
                            ],
                            structured_content={
                                "success": False,
                        ...
                        # For dry runs, show as warning
                        migration_steps.append(
                            f"⚠️  (DRY RUN) Migration would fail due to port conflicts: {', '.join(map(str, conflicted_ports))}"
                        )
            else:
            ...
            if total_names > 0:
                migration_steps.append(
                    f"   Expected containers: {len(expected_names['containers'])}, networks: {len(expected_names['networks'])}"
                )
                (
                    no_name_conflicts,
                    name_conflicts,
                    conflict_details,
                ) = await self._check_name_conflicts(target_host, expected_names)
                ...
                if no_name_conflicts:
                    ...
                    migration_steps.append(
                        f"   ❌ Name conflicts on {target_host.hostname}: {', '.join(conflict_summary)}"
                    )

                    # Show specific conflicting names
                    if name_conflicts["containers"]:
                        migration_steps.append(
                            f"     Containers: {', '.join(name_conflicts['containers'][:3])}{'...' if len(name_conflicts['containers']) > 3 else ''}"
                        )
                    if name_conflicts["networks"]:
                        migration_steps.append(
                            f"     Networks: {', '.join(name_conflicts['networks'][:3])}{'...' if len(name_conflicts['networks']) > 3 else ''}"
                        )
                    ...
                    if not dry_run:
                        ...
                        return ToolResult(
                            content=[
                                TextContent(
                                    type="text",
                                    text=f"❌ Migration blocked: Name conflicts on target host. {len(all_conflicts)} conflicting names found.",
                                )
                            ],
                            structured_content={
                                "success": False,
                        ...
                        all_conflicts = name_conflicts["containers"] + name_conflicts["networks"]
                        migration_steps.append(
                            f"⚠️  (DRY RUN) Migration would fail due to name conflicts: {len(all_conflicts)} conflicts"
                        )
            else:
            ...
            if dry_run:
                ...
                connectivity_ok, connectivity_details = await self._test_network_connectivity(
                ...
                if ssh_tests.get("source_ssh", {}).get("success"):
                    migration_steps.append(
                        f"   ✅ SSH to source ({source_host.hostname}): Connected"
                    )
                else:
                    error = ssh_tests.get("source_ssh", {}).get("error", "Unknown error")
                    migration_steps.append(
                        f"   ❌ SSH to source ({source_host.hostname}): Failed - {error}"
                    )

                if ssh_tests.get("target_ssh", {}).get("success"):
                    migration_steps.append(
                        f"   ✅ SSH to target ({target_host.hostname}): Connected"
                    )
                else:
                    error = ssh_tests.get("target_ssh", {}).get("error", "Unknown error")
                    migration_steps.append(
                        f"   ❌ SSH to target ({target_host.hostname}): Failed - {error}"
                    )
                ...
                if speed_test.get("success"):
                    ...
                    migration_steps.append(
                        f"   ✅ Network speed test: {speed} ({transfer_time} for 1MB)"
                    )
                else:
                ...
                if not connectivity_ok:
                    migration_steps.append(
                        "⚠️  (DRY RUN) Network connectivity issues detected - migration may be slow or fail"
                    )
                ...
                if estimated_size > 0:
                    ...
                    network_speed_test = connectivity_details.get("tests", {}).get(
                        "network_speed", {}
                    )
                    time_estimates = self._estimate_transfer_time(
                        estimated_size, network_speed_test
                    )
                    ...
                    if "actual_network" in time_estimates["estimates"]:
                        ...
                        migration_steps.append(
                            f"   📊 Estimated time: {actual_est['time_with_overhead_human']} (with overhead)"
                        )
                        migration_steps.append(
                            f"     Based on measured speed: {actual_est['speed']}"
                        )
                    else:
                        ...
                        for est_name in ["100_mbps", "1_gbps"]:
                            if est_name in time_estimates["estimates"]:
                                ...
                                migration_steps.append(
                                    f"     • {est['description']}: {est['time_with_overhead_human']}"
                                )

                    migration_steps.append(
                        f"     Raw data: {time_estimates['data_size_human']} → Compressed: ~{time_estimates['compressed_size_human']}"
                    )

                    # Add warnings for large transfers
                    if (
                        time_estimates["estimates"].get("100_mbps", {}).get("time_seconds", 0)
                        > 3600
                    ):  # > 1 hour
                        migration_steps.append(
                            "⚠️  Large dataset detected - consider migrating during off-peak hours"
                        )
                else:
                    migration_steps.append(
                        "⏱️ No significant data to transfer - minimal time required"
                    )
                ...
                if "actual_network" in time_estimates.get("estimates", {}):
                    transfer_time = time_estimates["estimates"]["actual_network"].get(
                        "time_with_overhead", 600
                    )
                elif "100_mbps" in time_estimates.get("estimates", {}):
                    transfer_time = time_estimates["estimates"]["100_mbps"].get(
                        "time_with_overhead", 600
                    )
                else:
                ...
                if risk_assessment["warnings"]:
                    ...
                    if len(risk_assessment["warnings"]) > 3:
                        migration_steps.append(
                            f"     • ... and {len(risk_assessment['warnings']) - 3} more warnings"
                        )

                if risk_assessment["critical_files"]:
                    migration_steps.append(
                        f"   🔥 Critical files detected: {len(risk_assessment['critical_files'])}"
                    )
                    ...
                    if len(risk_assessment["critical_files"]) > 2:
                        migration_steps.append(
                            f"     • ... and {len(risk_assessment['critical_files']) - 2} more"
                        )
                ...
                if risk_assessment["recommendations"]:
                    ...
                    if len(risk_assessment["recommendations"]) > 3:
                        migration_steps.append(
                            f"     • ... and {len(risk_assessment['recommendations']) - 3} more"
                        )

                migration_steps.append(
                    f"   📋 Estimated downtime: {self._format_time(estimated_downtime)}"
                )

                # Show rollback plan for medium/high risk
                if risk_level in ["MEDIUM", "HIGH"]:
                    migration_steps.append(
                        "   🔄 Rollback plan available - review before proceeding"
                    )
            else:
                ...
                try:
                    ...
                    source_ssh_cmd = build_ssh_command(source_host) + ["echo 'QUICK_CHECK'"]
                    target_ssh_cmd = build_ssh_command(target_host) + ["echo 'QUICK_CHECK'"]
                    ...
                    if source_result.returncode == 0 and target_result.returncode == 0:
                        ...
                        migration_steps.append(
                            f"   ❌ SSH connectivity failed for: {', '.join(failed_hosts)}"
                        )
                        ...
                        return ToolResult(
                            content=[
                                TextContent(
                                    type="text",
                                    text=f"❌ Migration blocked: SSH connectivity failed for {', '.join(failed_hosts)}",
                                )
                            ],
                            structured_content={
                                "success": False,
                    ...
                    return ToolResult(
                        content=[
                            TextContent(
                                type="text",
                                text=f"❌ Migration blocked: SSH connectivity check failed: {str(e)}",
                            )
                        ],
                        structured_content={
                            "success": False,
            ...
            if not skip_stop_source and not dry_run:
                ...
                if not stop_result["success"]:
                    return ToolResult(
                        content=[
                            TextContent(
                                type="text",
                                text=f"Error: Failed to stop source stack: {stop_result.get('error')}",
                            )
                        ],
                ...
                verify_cmd = ssh_cmd_source + [
                    (
                        f"docker ps --filter "
                        f"'label={DOCKER_COMPOSE_PROJECT}={shlex.quote(stack_name)}' "
                        f"--format '{{{{.Names}}}}'"
                    )
                ]
            ...
            if all_paths:
                ...
                migration_steps.append(
                    ...
                    f"{format_size(source_inventory['total_size'])}, "
            ...
            if all_paths:
                if not dry_run:
                    ...
                    verify_archive_cmd = ssh_cmd_source + [
                    ...
                    if "FAILED" in verify_archive.stdout:
                        return ToolResult(
                            content=[
                                TextContent(
                                    type="text",
                                    text="Error: Archive verification failed. The archive may be corrupted.",
                                )
                            ],
                            structured_content={
                                "success": False,
                    ...
                    if source_inventory:
                        estimated_size = int(
                            source_inventory["total_size"] * 0.3
                        )  # Estimate 70% compression
                        migration_steps.append(
                            f"📦 (DRY RUN) Would create archive: ~{format_size(estimated_size)}"
                        )
                    else:
            ...
            if all_paths and len(all_paths) > 0:
                ...
                relative_path = source_path.replace(source_appdata + "/", "").rstrip(
                    "/"
                )  # memos/.memos
            ...
            check_dir_cmd = (
                f"test -d {shlex.quote(target_stack_dir)} && echo 'EXISTS' || echo 'NOT_EXISTS'"
            )
            check_result = await loop.run_in_executor(
                None,
                lambda: subprocess.run(
            ...
            if not dry_run:
                    ...
                    if result.returncode != 0:
                        return ToolResult(
                            content=[
                                TextContent(
                                    type="text",
                                    text=f"❌ Failed to create target directory: {result.stderr}",
                                )
                            ],
                            structured_content={
                                "success": False,
                                "error": f"Target directory creation failed: {result.stderr}",
                            },
                        )
                ...
                if directory_exists:
                    migration_steps.append(
                        f"ℹ️ (DRY RUN) Target directory exists: {target_stack_dir}"
                    )
                else:
                    migration_steps.append(
                        f"📁 (DRY RUN) Would create directory: {target_stack_dir}"
                    )
            ...
            if not dry_run:
                ...
                try:
                    ...
                    transfer_type, _ = await self.migration_manager.choose_transfer_method(
                        source_host, target_host
                    )
                    ...
                    if transfer_type == "zfs" and target_host.zfs_dataset:
                        ...
                        backup_info = await self.backup_manager.backup_zfs_dataset(
                            target_host,
                            target_host.zfs_dataset,
                            stack_name,
                            f"Pre-migration backup before {stack_name} migration from {source_host_id}",
                        )
                        migration_steps.append(
                            f"✅ ZFS backup created: {backup_info['snapshot_name']} ({backup_info['backup_size_human']})"
                        )
                    else:
                        # Check if directory has existing files before backing up
                        check_existing_cmd = build_ssh_command(target_host) + [
                            f"find {target_stack_dir} -type f 2>/dev/null | head -1"
                        ]
                        check_result = await loop.run_in_executor(
                            None,
                            lambda: subprocess.run(
                        ...
                        if has_existing_data:
                            ...
                            if backup_info.get("success") and backup_info.get("backup_path"):
                                migration_steps.append(
                                    f"✅ Backed up existing data: {backup_info['backup_path']} ({backup_info['backup_size_human']})"
                                )
                            else:
                    ...
                    migration_steps.append(
                        f"⚠️  Backup failed: {str(e)} - continuing with migration (RISKY)"
                    )
            ...
            if all_paths and archive_path:
                ...
                if not dry_run:
                    ...
                    if transfer_result["success"]:
                        ...
                        archive_size_cmd = ssh_cmd_source + [
                            f"stat -c%s {archive_path} 2>/dev/null || echo 0"
                        ]
                        ...
                        migration_steps.append(
                            f"📦 Archive ready: {format_size(archive_size)} at {archive_path}"
                        )
                else:
                    # Dry run: Simulate transfer with real data from inventory
                    if source_inventory:
                        migration_steps.append(
                            f"🚀 (DRY RUN) Would transfer {format_size(source_inventory['total_size'])}"
                        )
                        migration_steps.append(f"   • Files: {source_inventory['total_files']:,}")
                        migration_steps.append(
                            f"   • Critical files: {len(source_inventory.get('critical_files', {}))}"
                        )
                ...
                if not dry_run:
                    ...
                    migration_steps.append(
                        "📦 (DRY RUN) Would extract data with atomic replacement..."
                    )

                if not dry_run:
                    # Check target directory before extraction
                    check_before_cmd = build_ssh_command(target_host) + [
                        f"find {target_stack_dir} -type f 2>/dev/null | wc -l"
                    ]
                ...
                migration_steps.append(
                    f"🔍 Pre-extraction: {files_before} files in {target_stack_dir}"
                )
                self.logger.info(
                    "Target directory state before extraction",
                    target_dir=target_stack_dir,
                ...
                if not dry_run:
                    ...
                    extract_to_staging_cmd = build_ssh_command(target_host) + [
                    ...
                    self.logger.info(
                        "Phase 1: Extracting to staging directory",
                    ...
                    self.logger.info(
                        "Phase 1: Extraction to staging completed",
                    ...
                    if extraction_result.stderr:
                        self.logger.warning(
                            "Phase 1 extraction stderr", stderr=extraction_result.stderr[:500]
                        )  # Limit log size

                    # Check if Phase 1 succeeded before proceeding
                    if (
                        extraction_result.returncode != 0
                        or "EXTRACTION_COMPLETE" not in extraction_result.stdout
                    ):
                        self.logger.error(
                            "Phase 1: Extraction to staging failed",
                            return_code=extraction_result.returncode,
                            stdout=extraction_result.stdout,
                            stderr=extraction_result.stderr,
                        )
                        migration_steps.append(
                            "❌ Phase 1 FAILED: Archive extraction to staging directory"
                        )
                        return ToolResult(
                            content=[
                                TextContent(
                                    type="text",
                                    text=f"❌ Archive extraction failed: {extraction_result.stderr}",
                                )
                            ],
                            structured_content={
                                "success": False,
                                "error": f"Extraction failed: {extraction_result.stderr}",
                            },
                        )

                    # PHASE 2: Verify staging directory has files
                    migration_steps.append(
                        "🔍 Phase 2: Verifying files extracted to staging directory..."
                    )
                    check_staging_cmd = build_ssh_command(target_host) + [
                        f"find {target_stack_dir}.tmp -type f 2>/dev/null | wc -l"
                    ]
                    ...
                    self.logger.info(
                        "Phase 2: Staging directory verification",
                        staging_dir=f"{target_stack_dir}.tmp",
                    ...
                    if staging_files_count == 0:
                        self.logger.error(
                            "Phase 2: No files found in staging directory",
                            staging_dir=f"{target_stack_dir}.tmp",
                        ...
                        migration_steps.append(
                            "❌ Phase 2 FAILED: No files found in staging directory"
                        )
                        migration_steps.append(
                            f"   Expected files: {source_inventory.get('total_files', 0) if source_inventory else 0}"
                        )
                        ...
                        return ToolResult(
                            content=[
                                TextContent(
                                    type="text",
                                    text="❌ Staging verification failed: No files extracted",
                                )
                            ],
                            structured_content={
                                "success": False,
                                "error": "No files found in staging directory",
                            },
                        )

                    migration_steps.append(
                        f"✅ Phase 2: Verified {staging_files_count} files in staging directory"
                    )

                    # PHASE 3: Atomic move from staging to final location
                    migration_steps.append(
                        "🔄 Phase 3: Performing atomic move to final location..."
                    )
                    atomic_move_cmd = build_ssh_command(target_host) + [
                        f"set -e && "
                    ...
                    if (
                        ...
                        or "ATOMIC_MOVE_SUCCESS" not in move_result.stdout
                    ):
                        self.logger.error(
                            "Phase 3: Atomic move failed",
                            return_code=move_result.returncode,
                        ...
                        return ToolResult(
                            content=[
                                TextContent(
                                    type="text", text=f"❌ Atomic move failed: {move_result.stderr}"
                                )
                            ],
                            structured_content={
                                "success": False,
                                "error": f"Atomic move failed: {move_result.stderr}",
                            },
                        )
                    ...
                    check_after_cmd = build_ssh_command(target_host) + [
                        f"find {target_stack_dir} -type f 2>/dev/null | wc -l"
                    ]
                    ...
                    self.logger.info(
                        "Split-phase extraction completed successfully",
                        files_before=files_before,
                    ...
                    migration_steps.append(
                        f"   • Phase 1: ✓ Archive extracted to staging ({staging_files_count} files)"
                    )
                    migration_steps.append("   • Phase 2: ✓ Staging directory verified")
                    migration_steps.append("   • Phase 3: ✓ Atomic move to final location")
                    migration_steps.append(
                        f"   • Final state: {files_after} files in {target_stack_dir}"
                    )
                else:
                    # Dry run: Show extraction plan with real data
                    if source_inventory:
                        migration_steps.append(
                            f"📦 (DRY RUN) Would extract {source_inventory['total_files']:,} files"
                        )
                        migration_steps.append(
                            f"   • Total size: {format_size(source_inventory['total_size'])}"
                        )
                        migration_steps.append(
                            f"   • Critical files: {len(source_inventory.get('critical_files', {}))}"
                        )
            ...
            if not dry_run and source_inventory:
                ...
                self.logger.info(
                    "Starting split-phase verification",
                    target_dir=target_stack_dir,
                ...
                data_verification_passed = (
                    extraction_result.returncode == 0  # Phase 1: Tar extraction succeeded
                    and "EXTRACTION_COMPLETE"
                    in extraction_result.stdout  # Phase 1: Marker confirmed
                    and staging_files_count > 0  # Phase 2: Files verified in staging
                    and move_result.returncode == 0  # Phase 3: Atomic move succeeded
                    and "ATOMIC_MOVE_SUCCESS"
                    in move_result.stdout  # Phase 3: Move marker confirmed
                )

                # Log the decision with detailed reasoning
                self.logger.info(
                    "Split-phase verification completed",
                ...
                data_verification = {
                    "data_transfer": {
                        ...
                        "file_match_percentage": (staging_files_count / expected_files * 100)
                        if expected_files > 0
                        else 0,
                ...
                if data_verification_passed:
                    migration_steps.append(
                        "✅ Verification PASSED: Split-phase extraction successful"
                    )
                    migration_steps.append(
                        f"   • Phase 1: ✓ Archive extracted (exit code {extraction_result.returncode})"
                    )
                    migration_steps.append(
                        f"   • Phase 2: ✓ Staging verified ({staging_files_count} files)"
                    )
                    migration_steps.append(
                        f"   • Phase 3: ✓ Atomic move completed (exit code {move_result.returncode})"
                    )
                else:
                    migration_steps.append(
                        "❌ Verification FAILED: Split-phase extraction incomplete"
                    )
                    migration_steps.append(f"   Expected files: {expected_files}")
                    migration_steps.append(
                        f"   Phase 1: {'✓' if extraction_result.returncode == 0 else '✗'} (exit {extraction_result.returncode})"
                    )
                    migration_steps.append(
                        f"   Phase 2: {'✓' if staging_files_count > 0 else '✗'} ({staging_files_count} staging files)"
                    )
                    migration_steps.append(
                        f"   Phase 3: {'✓' if move_result.returncode == 0 else '✗'} (exit {move_result.returncode})"
                    )
                    migration_steps.append(
                        "⛔ Stack deployment CANCELLED - split-phase extraction failed"
                    )
                        ...
                        try:
                            if backup_info["type"] == "zfs_snapshot":
                                (
                                    rollback_success,
                                    rollback_msg,
                                ) = await self.backup_manager.restore_zfs_backup(
                                    target_host, backup_info
                                )
                            else:
                                (
                                    rollback_success,
                                    rollback_msg,
                                ) = await self.backup_manager.restore_directory_backup(
                                    target_host, backup_info
                                )
                ...
                if dry_run and source_inventory:
                    ...
                    migration_steps.append(
                        f"   • Expected files: {source_inventory['total_files']:,}"
                    )
                    migration_steps.append(
                        f"   • Expected size: {format_size(source_inventory['total_size'])}"
                    )
            ...
            if start_target and not dry_run and data_verification_passed:
                ...
                if deploy_result["success"]:
                    ...
                    expected_volumes = self._extract_expected_mounts(
                        updated_compose, target_appdata, stack_name
                    )
                    container_verification = (
                        await self.migration_manager.verify_container_integration(
                            build_ssh_command(target_host),
                            stack_name,
                            target_appdata,
                            expected_volumes,
                        )
                    ...
                    verification_results = {
                        "data_transfer": data_verification["data_transfer"]
                        if not dry_run and source_inventory
                        else {"success": True},
                        "container_integration": container_verification["container_integration"],
                        "all_issues": (
                            data_verification["issues"] if not dry_run and source_inventory else []
                        )
                        + container_verification["issues"],
                        "overall_success": data_verification_passed
                        and deploy_success
                        and container_verification["container_integration"]["success"],
                    }

                    if container_verification["container_integration"]["success"]:
                        migration_steps.append("✅ Container Integration Verified:")
                        migration_steps.append(
                            f"   • Container: ✓ {'Running & Healthy' if container_verification['container_integration']['container_healthy'] else 'Running'}"
                        )
                        migration_steps.append(
                            f"   • Mounts: ✓ {'Correctly Mounted' if container_verification['container_integration']['mount_paths_correct'] else 'Mounted'}"
                        )
                        migration_steps.append(
                            f"   • Data Access: ✓ {'Accessible' if container_verification['container_integration']['data_accessible'] else 'Limited'}"
                        )
                    else:
                    ...
                    migration_steps.append(
                        f"❌ Failed to deploy on target: {deploy_result.get('error')}"
                    )
                    verification_results = {
                        "data_transfer": data_verification["data_transfer"]
                        if not dry_run and source_inventory
                ...
                if volumes_info:
                    migration_steps.append(
                        f"   • Named volumes: {len(volumes_info['named_volumes'])}"
                    )
                    ...
                    if len(volumes_info["named_volumes"]) > 3:
                        migration_steps.append(
                            f"     ... and {len(volumes_info['named_volumes']) - 3} more"
                        )
                ...
                verification_results = {
                    ...
                    "container_integration": {"success": False},
                    "all_issues": data_verification["issues"]
                    if not dry_run and source_inventory
            ...
            if (
                ...
                and verification_results["overall_success"]
            ):
                migration_steps.append(
                    f"🗑️  Migration successful - removing stack from {source_host_id}..."
                )
                ...
                migration_steps.append(
                    "⚠️  (DRY RUN) Would skip source removal - simulated migration verification"
                )
            ...
            return ToolResult(
                ...
                structured_content={
                    ...
                    "transfer_stats": transfer_result.get("stats", {})
                    if "transfer_result" in locals()
                    else {},
                    "source_summary": {
                        ...
                        "total_size_human": format_size(source_inventory["total_size"])
                        if source_inventory
                        else "0 B",
                        "critical_files": len(source_inventory.get("critical_files", {}))
                        if source_inventory
                        else 0,
                    },
                    "verification_summary": {
                        "data_transfer": {
                            "files_match": f"{verification_results['data_transfer']['file_match_percentage']:.1f}%"
                            if verification_results
                            else "N/A",
                            "size_match": f"{verification_results['data_transfer']['size_match_percentage']:.1f}%"
                            if verification_results
                            else "N/A",
                            "critical_verified": sum(
                                1
                                for v in verification_results["data_transfer"][
                                    "critical_files_verified"
                                ].values()
                                if v.get("verified")
                            )
                            ...
                            "critical_total": len(
                                verification_results["data_transfer"]["critical_files_verified"]
                            )
                            if verification_results
                            else 0,
                            "issues": len(verification_results.get("all_issues", []))
                            if verification_results
                            else 0,
                        },
                        "container": {
                            "running": verification_results["container_integration"].get(
                                "container_running", False
                            )
                            if verification_results
                            else False,
                            "healthy": verification_results["container_integration"].get(
                                "container_healthy", False
                            )
                            if verification_results
                            else False,
                            "mounts_correct": verification_results["container_integration"].get(
                    ...
                    "backup_summary": {
                        ...
                        and backup_info.get("success", False),
                        "backup_type": backup_info.get("type") if backup_info else None,
                        "backup_path": backup_info.get("backup_path")
                        if backup_info and backup_info.get("type") == "directory"
                        else None,
                        "backup_snapshot": backup_info.get("snapshot_name")
                        if backup_info and backup_info.get("type") == "zfs_snapshot"
                        else None,
                        "backup_size": backup_info.get("backup_size", 0) if backup_info else 0,
                        "backup_size_human": backup_info.get("backup_size_human", "0 B")
                        if backup_info
                        else "0 B",
    ...
    async def _check_port_conflicts(self, host, ports: list[int]) -> tuple[bool, list[int], dict]:
        ...
        for port in ports:
            try:
                ...
                check_cmd = ssh_cmd + [
                    f"ss -tuln | grep ':{port}' >/dev/null 2>&1 && echo 'IN_USE' || echo 'AVAILABLE'"
                ]
    ...
    def _extract_names_from_compose(
        ...
        try:
            ...
            return {"containers": container_names, "networks": network_names}

        except Exception as e:
            self.logger.warning(
                "Failed to parse container/network names from compose file", error=str(e)
            )
            return {"containers": [], "networks": []}

    async def _check_name_conflicts(
        self, host, expected_names: dict[str, list[str]]
        ...
        if container_names:
            for container_name in container_names:
                try:
                    ...
                    check_cmd = ssh_cmd + [
                        f"docker ps -a --format '{{{{.Names}}}}' | grep -x '{shlex.quote(container_name)}' >/dev/null 2>&1 && echo 'EXISTS' || echo 'AVAILABLE'"
                    ]
        ...
        if network_names:
            for network_name in network_names:
                try:
                    ...
                    check_cmd = ssh_cmd + [
                        f"docker network ls --format '{{{{.Name}}}}' | grep -x '{shlex.quote(network_name)}' >/dev/null 2>&1 && echo 'EXISTS' || echo 'AVAILABLE'"
                    ]
            ...
            if ssh_tests["source_ssh"]["success"] and ssh_tests["target_ssh"]["success"]:
                try:
                    ...
                    create_test_file_cmd = source_ssh_cmd[:-1] + [
                        "dd if=/dev/zero of=/tmp/speed_test bs=1M count=1 2>/dev/null && echo 'FILE_CREATED'"
                    ]
                    result = await loop.run_in_executor(
                        None,
                        lambda: subprocess.run(
                    ...
                    if result.returncode == 0 and "FILE_CREATED" in result.stdout:
                        ...
                        if result.returncode == 0:
                            ...
                            speed_mbps = (
                                (1.0 / transfer_time) * 8 if transfer_time > 0 else 0
                            )  # Convert to Mbps
                            speed_test = {
                                "success": True,
                                "transfer_time": f"{transfer_time:.2f}s",
                                "estimated_speed": f"{speed_mbps:.1f} Mbps"
                                if speed_mbps > 0
                                else "N/A",
                                "file_size": "1 MB",
                            }
                        ...
                        speed_test = {"success": False, "error": "Failed to create test file"}

                except Exception as e:
                    speed_test = {"success": False, "error": f"Speed test error: {str(e)}"}
            else:
                speed_test = {"success": False, "error": "SSH connectivity required for speed test"}

            details["tests"]["network_speed"] = speed_test

            # Overall connectivity assessment
            connectivity_ok = (
                ssh_tests["source_ssh"]["success"]
                and ssh_tests["target_ssh"]["success"]
                and speed_test.get("success", False)
            )
                            ...
                            for _i, port_spec in enumerate(ports):
                                if isinstance(port_spec, str):
                                    ...
                                    if "target" not in port_spec:
                                        service_issues.append(
                                            f"Service '{service_name}': Port mapping missing 'target'"
                                        )
